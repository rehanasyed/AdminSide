...
//  RisingStar
//
//  Created by Macbook Pro on 02/03/2019.
//  Copyright © 2019 iOS Troopers. All rights reserved.
//
    ...
    override func awakeFromNib() {
        ...
        if (txtImage != nil)
        {
            ...
            if UIDevice.init().userInterfaceIdiom == UIUserInterfaceIdiom.pad{
                let adjustmentFactor = Int( Float(imageWidth) * 0.5)
                imageWidth += adjustmentFactor
            }
            imgView.frame = CGRect(x: 0, y: 0, width: CGFloat(imageWidth), height: self.frame.height*(CGFloat(imageWidth)/self.frame.width))
